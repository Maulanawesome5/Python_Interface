...
status = Label(master=root, text="Image 1 of " + str(len(image_list)), bd=1, relief=SUNKEN, anchor=E)
...
def forward(image_number):
    ...
    status = Label(master=root, text="Image "+ str(image_number) + " of " + str(len(image_list)), bd=1, relief=SUNKEN, anchor=E)
...
button_next = Button(master=root, text=">>", command=lambda: forward(2)).grid(row=1, column=2, pady=10)
status.grid(row=2, column=0, columnspan=3, sticky=W+E)
